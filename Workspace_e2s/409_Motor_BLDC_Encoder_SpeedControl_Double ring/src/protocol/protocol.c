...
static struct prot_frame_parser_t parser = {0};
...
static uint8_t protocol_frame_parse(uint8_t *data, uint16_t *data_len)
{
    ...
    int32_t need_to_parse_len = 0;
    int32_t header_oft = -1;
    uint8_t checksum = 0;
...
//        set_p_i_d(p_temp, i_temp, d_temp);    // 设置 P I D
      ...
      case SET_TARGET_CMD:
      {
        ...
        set_pid_target(&pid_location, actual_temp);    // 设置目标值
...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  ...
  uart_protocol_send((uint8_t *)&set_packet, sizeof(set_packet));    // 发送数据头
  uart_protocol_send((uint8_t *)data, num);                          // 发送参数
  uart_protocol_send((uint8_t *)&sum, sizeof(sum));                  // 发送校验和
}
