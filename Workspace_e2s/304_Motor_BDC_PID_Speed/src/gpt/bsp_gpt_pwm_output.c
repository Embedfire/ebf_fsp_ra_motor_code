...
void Motor_GPT_PWM_Init(void)
{
    ...
    R_GPT_Enable(&g_timer5_pwm_ctrl);
...
void Motor_GPT_PWM_SetDuty(float duty_pwm1, float duty_pwm2)
{
    ...
    duty_cycle_counts1 = (uint32_t)(((float) (current_period_counts1 - 100) * duty_pwm1) / (float)100);
    duty_cycle_counts2 = (uint32_t)(((float) (current_period_counts2 - 100) * duty_pwm2) / (float)100);
