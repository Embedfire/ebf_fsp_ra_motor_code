...
{
    X_STEP_ENABLED; // 使能电机
    R_GPT_Start(&x_step_pwm_ctrl);// 使能定时器7的比较输出
...
{
    Y_STEP_ENABLED; // 使能电机
    R_GPT_Start(&y_step_pwm_ctrl);// 使能定时器7的比较输出
}
...
{
    X_STEP_DISABLED;              // 禁用电机
    R_GPT_Stop(&x_step_pwm_ctrl);  // 禁用定时器7的PWM输出
}
...
{
    Y_STEP_DISABLED;              // 禁用电机
    R_GPT_Stop(&y_step_pwm_ctrl);  // 禁用定时器7的PWM输出
}
...
void Motor_Control_Reverse(void)
{
    ...
    X_STEP_DIRECTION_TOGGLE;      // 切换电机方向控制引脚
}
