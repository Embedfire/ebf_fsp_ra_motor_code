...
void motor2_gpt_init(void)
{
    ...
    /* 初始化PWM定时器 */
    err = R_GPT_Open(&motor2_u_ctrl, &motor2_u_cfg);
    assert(FSP_SUCCESS == err);

    err = R_GPT_Open(&motor2_v_ctrl, &motor2_v_cfg);
    assert(FSP_SUCCESS == err);

    err = R_GPT_Open(&motor2_w_ctrl, &motor2_w_cfg);
    assert(FSP_SUCCESS == err);

    /* 启动 PWM 定时器 */
    err = R_GPT_Start(&motor2_u_ctrl);
    assert(FSP_SUCCESS == err);

    err = R_GPT_Start(&motor2_v_ctrl);
    assert(FSP_SUCCESS == err);

    err = R_GPT_Start(&motor2_w_ctrl);
    assert(FSP_SUCCESS == err);
